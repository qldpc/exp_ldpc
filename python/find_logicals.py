...
def find_logicals(checks : QuantumCodeChecks) -> QuantumCodeLogicals:
    ...
    def find_logical_single_basis(checks, conjugate_checks):
        ...
        conjugate_submodule = module.submodule(matrix_to_module_elements(sparse_to_f2_matrix(conjugate_checks), module_basis))
        logical_support = (frozenset(g.support()) for g in frozenset(conjugate_submodule.reduce(g) for g in matrix_to_module_elements(kernel, module_basis)) if len(g.monomials()) > 0)
        return make_check_matrix(list(logical_support), num_qubits)

    return (find_logical_single_basis(z_checks, x_checks), find_logical_single_basis(x_checks, z_checks), num_qubits)
...
def test_find_logicals():
    ...
    assert list(sparse_to_support(logicals[0])) == [{8, 6, 7}]
    assert list(sparse_to_support(logicals[1])) == [{8, 2, 5}]
